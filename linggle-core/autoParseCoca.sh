...
# 設定根目錄
ROOT_DIR="/home/nlplab/atwolin/Linggle-EMI/yale-ocw2"
RESULT_DIR="/home/nlplab/atwolin/Linggle-EMI"
OUTPUT_FOLDER_NAME="yale-ocw-parse-line"
  ...
  for COURSE_NAME in */; do
    ...
    for LECTURE_FILE in *.txt; do
      ...
      if [ -f "$LECTURE_FILE" ]; then
        ...
        OUTPUT_FILE_PATH="$RESULT_DIR/$OUTPUT_FOLDER_NAME/$DEPT_NAME/$COURSE_NAME/${LECTURE_FILE%.txt}_parse.txt"
        ...
        python /home/nlplab/atwolin/Linggle-EMI/linggle-core/linggle/parse/parse_coca.py "$FILE_PATH" > $OUTPUT_FILE_PATH
