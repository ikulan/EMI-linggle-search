...
class LinggleCommand:
    ...
    def gen_candidates(self, query):
        for item in QUERY_RE.findall(query):
            if item == "*":
                yield (
                    "",
                    *("_".join(" " * (n + 1)) for n in range(1, LONGEST_LEN + 1)),
                )
            elif item.startswith("{") and item.endswith("}"):
                items = QUERY_RE.findall(item[1:-1])
                yield tuple(
                    cmd
                    for tokens in permutations(items)
    ...
    def candidates_to_cmds(self, candidates, return_str=True):
        for tokens in product(*candidates):
            ...
            tokens = (
                self.word_delimiter.join(token for token in tokens if token)
                .strip()
                .split()
            )
    ...
    def expand_query(self, querystr, return_str=True):
        ...
        linggle_cmds = tuple(
            {cmd for cmd in self.candidates_to_cmds(candidates, return_str)}
        )
